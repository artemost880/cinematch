...
class _RandomMovieScreenState extends State<RandomMovieScreen> {
  ...
  String _currentGenresQuery = ''; // ИСПРАВЛЕНО: Храним жанры как строку для TMDB
  ...
  Future<void> _fetchRandomMovie() async {
    ...
    final movie = await _tmdbService.getRandomMovie(
      ...
      genreIds: _currentGenresQuery, // ИСПРАВЛЕНО: Передаем строку, а не список, null больше не будет
    ...
    if (movie != null) {
      // ИСПРАВЛЕНО: Используем новый метод получения рейтинга Кинопоиска по названию и году
      final String? releaseYear = movie['release_date']?.split('-')[0];
      _fetchKinopoiskRating(movie['title'], releaseYear);
    }
  ...
  Future<void> _fetchKinopoiskRating(String title, String? releaseYear) async {
            ...
            onPressed: () async {
              ...
              if (result != null) {
                setState(() {
                  ...
                  _currentGenresQuery = result['genresQuery'] ?? ''; // ИСПРАВЛЕНО: Сохраняем строку genresQuery
